...
@endsection
@section('mycss')
    <link rel="stylesheet" href="/frontend/css/reviewDetail.css">
...
@section('content')
    ...
    <section class="container customerExperience-content">
        <div class="col-md-9 customerExperience">
            <h3>{{$review->title}}</h3>
            <div class="customer-card">
                <div class="customer-card__img">
                    <a href=""><img src="{{asset($review->avatar)}}" alt=""></a>
                </div>
                <div class="customer-card__desc">
                    <strong>{{$review->name}}</strong> đã tham gia:
                    <strong>
                        @foreach($customers as $customer)
                            @if($customer->name == $review->name)
                            {{$customer->tour->name}} <a href="{{route('home.tourDetail',['slug'=>$customer->tour->slug])}}">(Xem tour ngay)</a>.
                            @endif
                        @endforeach
                    </strong>
                    <p>{{$review->address}}</p>

                    <div class="customerExperience-rating">
            ...
            <div class="customerReviews">
                <h5>Các đánh giá cùng tour</h5>
                <a href="">> VietCenter thật tuyệt vời</a>
            </div>
